...
def ros_quat_to_euler(ros_quat):
    return Rt.from_quat(ros_quat).as_euler("XYZ")

def ros_quat_to_rotvec(ros_quat):
    return Rt.from_quat(ros_quat).as_rotvec()
...
if __name__ == "__main__":
    pos_quat = np.random.rand(10, 7)

    code.interact(local=dict(globals(), **locals()))

    pos_euler_answer1 = mat_to_pos_euler(pos_ros_quat_to_mat(pos_quat[:, :3], pos_quat[:, 3:])[9])
    pos_euler_answer2 = np.concatenate((pos_quat[:, :3], ros_quat_to_euler(pos_quat[:, 3:])), axis = 1)
