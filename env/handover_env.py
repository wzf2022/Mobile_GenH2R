...
def debug():
    env_base_cfg = OmegaConf.structured(GenH2RSimConfig)
    cli_cfg = OmegaConf.from_cli()
    env_cfg = OmegaConf.to_object(OmegaConf.merge(env_base_cfg, cli_cfg))
    env = GenH2RSim(env_cfg)
    while True:
...
"""
DISPLAY="localhost:11.0" python -m env.handover_env verbose=True panda.IK_solver=PyKDL visualize=True
