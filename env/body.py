...
class BodyConfig:
    ...
    set_human_hand_obj_last_frame: bool = False
...
class Body:
    ...
    def set_dof_target(self, dof_target_position=None, dof_target_velocity=None):
        # code.interact(local=dict(globals(), **locals()))
        # get_joint_positions
        # print(dof_target_position[:3] - self.get_joint_positions()[:3])
    ...
    def get_link_poses(self) -> NDArray[np.float64]:
        link_states = self.get_link_states()
        link_poses = []
        for link_state in link_states:
            pos, orn = link_state[4:6]
            link_poses.append(pos_ros_quat_to_mat(pos, orn))
        return np.stack(link_poses, axis=0)
