...
class StatusChecker:
    ...
    def check_object_release(self, target_object: Object, galbot: Galbot, frame: int):
        object_id, galbot_id = target_object.body_id, galbot.body_id # if hand is not loaded, hand.body_id = None
        contact = self.contact[self.contact["force"] > self.cfg.release_force_thresh].copy()

        if len(contact) == 0:
            contact_panda_release_region = [False] * len(galbot.fingers_link_id)
            contact_panda_body = False
        else:
            contact_1 = contact[(contact["body_id_a"] == object_id) & (contact["body_id_b"] == galbot_id)]
            contact_2 = contact[(contact["body_id_a"] == galbot_id) & (contact["body_id_b"] == object_id)]
            ...
            for link_id in galbot.fingers_link_id:
                ...
                if np.any(np.isnan(contact_link["position_b_link"]["x"])):
                    pos, orn = galbot.get_link_pos_orn(link_id)
        ...
        target_real_displacement = False
    ...
    def check_panda_hand_collision(self, galbot: Galbot, hand: Hand):
        panda_id, hand_id = galbot.body_id, hand.body_id # if hand is not loaded, hand.body_id = None
        ...
        if contact.shape[0] > 0 and contact["force"].max() > self.cfg.contact_force_thresh:
            if self.cfg.verbose: print("detect robot and  hand collision")
            return True
        else:
            return False

    def check_panda_humanoid_collision(self, galbot: Galbot, humanoid: Humanoid):
        panda_id, humanoid_id = galbot.body_id, humanoid.body_id # if hand is not loaded, hand.body_id = None
        contact_1 = self.contact[(self.contact["body_id_a"] == humanoid_id) & (self.contact["body_id_b"] == panda_id)]
        contact_2 = self.contact[(self.contact["body_id_a"] == panda_id) & (self.contact["body_id_b"] == humanoid_id)]
        contact = np.concatenate((contact_1, contact_2))
        if contact.shape[0] > 0 and contact["force"].max() > self.cfg.contact_force_thresh:
            if self.cfg.verbose: print("detect robot and humanoid collision")
            return True
        else:
            return False
        
    def check_object_dropped(self, objects: Objects, galbot: Galbot) -> Tuple[bool, bool]:
        if self.dropped:
            return True, False
        ...
        contact_panda = contact[contact["body_id_b"] == galbot.body_id]
        ...
        if not contact_panda_fingers and contact_other_objects:
            if self.cfg.verbose:
    ...
    def check_success(self, galbot: Galbot, contact_panda_fingers: bool):
        if not contact_panda_fingers:
            self.success_step_counter = 0
            return 0
        pos = galbot.get_link_pos(galbot.ee_link_id)
    ...
    def post_step(self, galbot: Galbot, hand: Hand, objects: Objects, humanoid: Humanoid, frame: int) -> Tuple[int, bool]:
        self.get_contact()
        release = not objects.released and self.check_object_release(objects.target_object, galbot, frame)
        # code.interact(local=dict(globals(), **locals()))
        status = 0
        if self.check_panda_hand_collision(galbot, hand) or self.check_panda_humanoid_collision(galbot, humanoid):
            status |= EpisodeStatus.FAILURE_HUMAN_CONTACT
        dropped, contact_panda_fingers = self.check_object_dropped(objects, galbot)
        if dropped:
            status |= EpisodeStatus.FAILURE_OBJECT_DROP
        if status != 0:
            return status, release
        if self.check_success(galbot, contact_panda_fingers):
